from rest_f"""Views for Bins and PickupRequest models."""
...
class PickupRequestViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated]
    filter_backends = [DjangoFilterBackend, SearchFilter, OrderingFilter]
    filterset_fields = ['status', 'payment_status', 'worker', 'owner']
    ...
    def stats(self, request):
        ...
        return Response(stats)k import viewsets, status
from rest_framework.decorators import action
from rest_framework.response import Response
from rest_framework.permissions import IsAuthenticated
from django.db.models import Q, Avg
from .models import Bin
from .serializers import BinSerializer, BinStatusUpdateSerializer


class BinViewSet(viewsets.ModelViewSet):
    """
    ViewSet for managing trash bins.

    Provides CRUD operations plus custom actions for:
    - Filtering bins by status
    - Getting bins that need pickup
    ...
    def get_queryset(self):
        ...
        if min_fill:
            queryset = queryset.filter(fill_level__gte=min_fill)

        # Filter by location (simple radius search)
        lat = self.request.query_params.get('lat')
        lng = self.request.query_params.get('lng')
        radius = self.request.query_params.get('radius', '0.01')  # Default ~1km
    ...
    @action(detail=False, methods=['get'])
    def needs_pickup(self, request):
        """Get all bins that need pickup."""
        bins = self.get_queryset().filter(
            Q(fill_level__gte=80) | Q(status=Bin.StatusChoices.FULL)
        )
        serializer = self.get_serializer(bins, many=True)
        return Response(serializer.data)
    ...
    @action(detail=True, methods=['patch'])
    def update_status(self, request, pk=None):
        """Update bin status from IoT sensors."""
        bin_obj = self.get_object()
        serializer = BinStatusUpdateSerializer(bin_obj, data=request.data, partial=True)
    ...
    @action(detail=False, methods=['get'])
    def stats(self, request):
        """Get bin statistics overview."""
        queryset = self.get_queryset()

        stats = {
            'total_bins': queryset.count(),
            'empty_bins': queryset.filter(status=Bin.StatusChoices.EMPTY).count(),
            'full_bins': queryset.filter(status=Bin.StatusChoices.FULL).count(),
            'needs_pickup': queryset.filter(Q(fill_level__gte=80) | Q(status=Bin.StatusChoices.FULL)).count(),
            'offline_bins': queryset.filter(is_online=False).count(),
            'average_fill_level': queryset.aggregate(avg_fill=Avg('fill_level'))['avg_fill'] or 0,
        }
