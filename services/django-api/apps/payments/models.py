...
class WorkerEarnings(models.Model):
    """Track worker earnings from completed pickups for dashboard."""
    ...
    pickup_request = models.OneToOneField('bins.PickupRequest', on_delete=models.CASCADE, related_name='worker_earning')

    # Earning details
    base_amount = models.DecimalField(max_digits=10, decimal_places=2, help_text="Base pickup fee")
    bonus_amount = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), help_text="Performance bonus")
    platform_fee = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), help_text="Platform commission")
    net_amount = models.DecimalField(max_digits=10, decimal_places=2, help_text="Final amount after fees")
    ...
    # Payout details
    payout_method = models.CharField(max_length=20, choices=PaymentTransaction.PaymentProvider.choices, default='mtn_money')
    ...
    def __str__(self):
        return f"{self.worker.username} - ${self.net_amount} ({self.get_status_display()})"

    def save(self, *args, **kwargs):
        """Calculate net amount automatically."""
        if not self.net_amount:
            self.net_amount = self.base_amount + self.bonus_amount - self.platform_fee
